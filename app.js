...
function renderDocuments(){
  ...
  ${docs.length ? docs.map(d=>`
    <div class="doc-row glass">
      ...
      <div class="meta"><div class="fn">${escapeHtml(d.name)}</div><div class="fx">${escapeHtml(d.type||'')} · ${new Date(d.uploadedAt).toLocaleDateString()}</div></div>
...
function wirePageEvents(page){
  ...
  if(page==='visionboard'){
    ...
    document.getElementById('visionInput').onchange = e=>{
      const file = e.target.files[0]; if(!file) return;
      const reader = new FileReader();
      reader.onload = ()=>{ addItem('visionBoard', { img: reader.result, caption:'' }); render(); toast('Added to vision board.','success'); };
      reader.readAsDataURL(file);
    };
    document.querySelectorAll('[data-del-vision]').forEach(b=> b.onclick = ()=>{ removeItem('visionBoard', b.dataset.delVision); render(); });
  }
  ...
  if(page==='documents'){
    ...
    document.getElementById('docInput').onchange = e=>{
      ...
      render(); toast('Document added.','success');
    };
    document.querySelectorAll('[data-del-doc]').forEach(b=> b.onclick = ()=> confirmDelete('Document', ()=>{ removeItem('documents', b.dataset.delDoc); render(); }));
  }
